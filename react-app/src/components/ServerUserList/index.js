...
export default function ServerUserList() {
  ...
  if (
    ...
    !Object.keys(serverDetails[serverId]).length
  ) {
    ...
    >
      ...
      <div className="conversation-user-container">
        <div className="dm-left">
          <img
            alt={`User icon for ${allUsers[owner[0]].username}`}
            className="dm-profile-img"
            src={allUsers[owner[0]].userIcon}
            style={ownerStatus ? { border: "2px solid green" } : {}}
          ></img>
          <p className="dm-username">
            {allUsers[owner[0]].username}
      ...
      {admins.map((userId) => {
        ...
        return (
          <div key={userId} className="conversation-user-container">
            <div className="dm-left">
              <img
                alt={`User icon for ${allUsers[userId].username}`}
                className="dm-profile-img"
                src={allUsers[userId].userIcon}

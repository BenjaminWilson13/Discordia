          ...
          return (
            <div key={server.id} className="server-container">
              <img
                ...
                className="explore-server-image"
                src={server.imageUrl}
              ></img>
              <div className="server-container-bottom">
                <div className="top-server-info">
                  <div className="server-username">
                    {displayName(server.name)}
                  </div>
                  {!(server.id in userServers) && (
                    <div>
                      <OpenModalButton
                        modalComponent={<JoinServerModal server={server} />}
                ...
                <div className="server-users">
                  {server.userCount} {server.userCount > 1 ? "users" : "user"}
                </div>
              </div>
            </div>
          );

...
export default function VoiceChannels() {
  ...
  async function newMemberAndOffer(data1) {
    ...
    navigator.mediaDevices
      ...
      .then(async (res) => {
        ...
        rtcPeers.current[data1.userId] = {
          offer: offer,
          offerPC: pc,
          offerer: { userId: currentUser.userId, channelId, serverId },
          answerer: data1,
        };
  ...
  function iceCandidate(data) {
    if (currentUser.userId !== data.to) {
      try {
        rtcPeers.current[data.to].answerPC.addIceCandidate(
          new RTCIceCandidate(data.candidate),
        );
      } catch (e) {
        console.error(e, "ice candidate error");
        rtcPeers.current[data.to].offerPC.addIceCandidate(
          new RTCIceCandidate(data.candidate),
        );
      }
    }
  ...
  function newJoinAndAnswer(data) {
    ...
    navigator.mediaDevices
      ...
      .then(async (res) => {
        ...
        rtcPeers.current[data.offerer.userId] = {
          ...data,
          answer,
          answerPC: pc,
          polite: true,
  ...
  useEffect(() => {
    ...
    socket.on("answer", async (data) => {
      ...
      await rtcPeers.current[data.answerer.userId].offerPC.setRemoteDescription(
  ...
  return (
    <div id="video-box" className="socket-container">
      ...
      <button onClick={clickEvent}>
        {videoToggle ? "Turn off Video" : "Start Video"}
      </button>
    </div>
  );
}

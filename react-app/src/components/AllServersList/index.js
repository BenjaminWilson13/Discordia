...
function AllServersList() {
  ...
  if (!serverList) return <div id="list-container"></div>;

  return (
    <div id="list-container">
      <div id="server-list-container">
        {serverList && serverList.length > 0 ? (
          <h1 id="home-title">Welcome back to Discordia!</h1>
        ) : (
          <div id="new-user-home">
            <h1>Welcome to Discordia! Join a server to begin...</h1>
            <button
              className="launch-server-button"
              id="server-list-explore"
              onClick={() => history.push("/servers/explore")}
            >
              Explore Servers
            </button>
          </div>
        ...
        {serverList.map((server) => {
          return (
            <div
              ...
              onClick={() =>
                history.push(
                  `/channels/${server.id}/${server.default_channel_id}`,
                )
              }
            >
              <img
              ...
              <div id="server-info">
                ...
                <p id="user-count">
                  {server.userCount} {server.userCount <= 1 ? "user" : "users"}
                </p>
              </div>

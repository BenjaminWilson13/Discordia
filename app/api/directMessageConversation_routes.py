...
def get_all_conversations():
    conversations = (
        DirectMessageConversationUser.query.with_entities(
            DirectMessageConversationUser.conversation_id
        )
        .filter(current_user.id == DirectMessageConversationUser.user_id)
        .all()
    )
    ...
    for conversation in conversation_ids:
        otherUsers = (
            DirectMessageConversationUser.query.with_entities(
                DirectMessageConversationUser.user_id
            )
            .filter(DirectMessageConversationUser.conversation_id == conversation)
            .all()
        )
        otherUser_id = [
            row.user_id for row in otherUsers if row.user_id != current_user.id
        ]
...
def create_direct_message(id):
    ...
    if form.validate():
        new_message_entire = DirectMessage(
            message=new_message, conversation_id=id, user_id=current_user.id
        )
        db.session.add(new_message_entire)
        db.session.commit()
        message = (
            DirectMessage.query.filter_by(conversation_id=id)
...
def create_direct_conversation():
    ...
    conversations_of_current_user = DirectMessageConversationUser.query.filter(
        DirectMessageConversationUser.user_id == current_user.id
    ).all()
    ...
    convos_new_person = DirectMessageConversationUser.query.filter(
        DirectMessageConversationUser.user_id == friend_person_dict["userId"]
    ).all()
    convo_ids_friend = []
    ...
    new_id = DirectMessageConversation.query.order_by(
    ...
    convo_user_1 = DirectMessageConversationUser(
        user_id=current_user.id, conversation_id=new_id["conversationId"]
    )
    db.session.add(convo_user_1)

    convo_user_2 = DirectMessageConversationUser(
        user_id=friend_person_dict["userId"], conversation_id=new_id["conversationId"]
    )
    ...
    new_id[str(friend_person_dict["userId"])] = {
        **user_friend,
        "conversation_id": new_id["conversationId"],
        "updated_at": new_id["updatedAt"],
    }
